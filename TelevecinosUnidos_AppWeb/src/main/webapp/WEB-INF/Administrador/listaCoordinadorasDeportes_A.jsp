    ...
    <div class="content">
        ...
        <div id="Titulo_Incidencias">
            <h1 class="display-4 text-center" style="margin-top:20px;margin-bottom:20px;padding: 20px">
                <h1 style="text-align: center"><i class="fas fa-chalkboard-teacher"></i> Lista de coordinadores de deporte</h1>
            </h1>
            <p class="lead text-center">Revisa la información de un coordinador</p>
        </div>
        <div style="background-color: #f8f9fa; padding: 10px; align-items: center;">
            <!-- Filtro -->
            <div style="background-color: #FFB703; padding: 20px; border: 1px solid #ccc; border-radius: 5px;">
                <div class="row justify-content-center align-items-center">
                    <form method="post" action="<%=request.getContextPath()%>/AdministradorServlet?action=buscarCoordiDeportesPorNombre">
                        <div class="col-md-7 mb-2">
                            <div class="input-group">
                                <input type="text" class="form-control" id="filtroInput" placeholder="Buscar coordinadora de deporte..." name="textoBuscar" value="<%=textoBusqueda%>"/>
                                <button class="btn btn-primary" type="submit">
                                    <i class="fas fa-search"></i> </button>
                                <a class="btn btn-secondary" href="<%=request.getContextPath()%>/AdministradorServlet?action=listaCoordinadorasDeportes_A">
                                    Limpiar
                                </a>
                            </div>
                        </div>
                ...
                <div class="table-responsive">
                    <table id="eventosTable" class="table" style="background-color: transparent;">
                        ...
                        <tr>
                            ...
                            <td>
                                <!--<a href="<%=request.getContextPath()%>/AdministradorServlet?action=banearCoordinadoraDeporte&idCoordinadora=<%=usuarioB.getIdUsuario()%>"><button type="button" class="btn btn-danger m-2" ><i class="fas fa-ban"></i></button></a>-->
                    ...
                    <nav>
                        <ul class="pagination">
                            <li class="page-item <%= paginaActual == 1 ? "disabled" : "" %>">
                                <a class="page-link" href="<%=request.getContextPath()%>/AdministradorServlet?action=listaCoordinadorasDeportes_A&page=<%= paginaActual - 1 %>">Anterior</a>
                            </li>
                            <% if (totalPaginas > 0) { %>
                            <% for (int i = 1; i <= totalPaginas; i++) { %>
                            <li class="page-item <%= i == paginaActual ? "active" : "" %>">
                                <a class="page-link" href="<%=request.getContextPath()%>/AdministradorServlet?action=listaCoordinadorasDeportes_A&page=<%= i %>"><%= i %></a>
                            </li>
                            <% } %>
                            <% } else { %>
                            ...
                            <li class="page-item <%= paginaActual == totalPaginas || totalPaginas == 0 ? "disabled" : "" %>">
                                ...
                                <a class="page-link" href="<%=request.getContextPath()%>/AdministradorServlet?action=listaCoordinadorasDeportes_A&page=<%= paginaActual + 1 %>">Siguiente</a>
                                <% } else { %>
        ...
        <script>
            ...
            function  banFunction(Id){
                window.location.href ='<%=request.getContextPath()%>/AdministradorServlet?action=banearCoordinadoraDeporte&idCoordinadora=' +Id;
            }
            function detalleFunction(Id){
                window.location.href ='<%=request.getContextPath()%>/AdministradorServlet?action=DetalleCoordinadoraDeportes_A&idCoordinadora=' +Id;
            }
        </script>
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    window.location.href = '<%= request.getContextPath() %>/AdministradorServlet?action=banearCoordinadoraDeporte&idCoordinadora=' + idUsuario;
                });
