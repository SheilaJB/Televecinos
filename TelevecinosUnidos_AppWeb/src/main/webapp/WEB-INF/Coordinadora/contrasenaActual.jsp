    ...
    <div id="navbar" class="content">
        ...
        <div class="container-fluid d-flex justify-content-center align-items-center vh-100">
            <div class="container text-center">
                ...
                <script>
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            window.location.href = "<%=request.getContextPath()%>/CoordinadorServlet?action=cambiarContrasena";
                        }
                    });
                </script>
                ...
                <% } %>
                <div class="container">
                    <h3 class="text-center">Verificación de cuenta</h3>
                    <label class="form-label"><b>Antes de proceder con el cambio de tu contraseña actual, por favor ingresa tus credenciales actuales</b></label>
                    <form method="post" action="<%= request.getContextPath() %>/CoordinadorServlet?action=contrasenaActual">
                        <div class="mb-3">
                            <label for="correo" class="form-label">Introduce tu correo gmail (*)</label>
                            <input type="text" class="form-control" id="correo" name="correo" required>
                        </div>
                        <div class="mb-3">
                            <label for="contrasenaActual" class="form-label">Introduce tu contraseña actual (*)</label>
                            <input type="password" class="form-control" id="contrasenaActual" name="contrasenaActual" required>
                        </div>

                        <button type="submit" class="button alert-success">Validar credenciales</button>
                    </form>
                    <a href="<%=request.getContextPath()%>/CoordinadorServlet?action=perfilC"><button class="button alert-danger">Regresar</button></a>
                </div>
            </div>
        </div>
    </div>
